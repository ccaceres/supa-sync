-- Fix bugs in bulk_calculate_opex_costs function
-- 1. Correct table names (salary_roles → exempt_positions, direct_roles → nonexempt_positions)
-- 2. Remove references to non-existent number_of_years column
        ...
        FOR year_idx IN 1..num_years LOOP
            ...
            BEGIN
                ...
                IF opex_record.driver_id IS NOT NULL THEN
                    ...
                    -- If not found, check exempt_positions (formerly salary_roles)
                    ...
                    -- If not found, check nonexempt_positions (formerly direct_roles)
                    IF driver_value IS NULL THEN

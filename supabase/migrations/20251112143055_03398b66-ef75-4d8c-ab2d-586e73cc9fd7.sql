-- Fix get_allocated_costs_for_price_line to include direct labor costs
-- This ensures the database RPC matches the client-side cost calculation logic

CREATE OR REPLACE FUNCTION get_allocated_costs_for_price_line(
...
BEGIN
  ...
  FOR allocation_record IN 
    SELECT 
      ...
      -- opex costs (calculated on-the-fly or stored)
      0 as opex_cost_1, 0 as opex_cost_2, 0 as opex_cost_3, 0 as opex_cost_4, 0 as opex_cost_5,
      0 as opex_cost_6, 0 as opex_cost_7, 0 as opex_cost_8, 0 as opex_cost_9, 0 as opex_cost_10,
      0 as opex_cost_11, 0 as opex_cost_12, 0 as opex_cost_13, 0 as opex_cost_14, 0 as opex_cost_15,
      0 as opex_cost_16, 0 as opex_cost_17, 0 as opex_cost_18, 0 as opex_cost_19, 0 as opex_cost_20,
      -- capex costs
    ...
    LEFT JOIN impex_lines imp ON cpa.cost_item_id = imp.id AND cpa.cost_type = 'impex'
  ...
  -- NEW: Add direct labor costs linked via price_line_id (100% allocation)
  -- These are NOT in cost_price_allocations, but directly linked to price_line
  FOR allocation_record IN 
    SELECT 
    ...
    WHERE price_line_id = p_price_line_id
      ...
      AND id NOT IN (
        ...
        WHERE price_line_id = p_price_line_id 
          AND cost_type = 'labex'
      )  -- Avoid double-counting if also in allocations table
  LOOP

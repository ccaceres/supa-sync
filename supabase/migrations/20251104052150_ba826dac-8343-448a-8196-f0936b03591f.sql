-- Mark bulk_calculate_impex_costs as VOLATILE to allow UPDATE operations
-- This fixes "cannot execute UPDATE in a read-only transaction" error
CREATE OR REPLACE FUNCTION public.bulk_calculate_impex_costs(p_model_id uuid)
RETURNS void
LANGUAGE plpgsql
VOLATILE  -- Explicitly mark as VOLATILE to allow write operations
SECURITY DEFINER
